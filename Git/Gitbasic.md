...
### I made a repo on github first and cloned into it to begin with, created a folder called gitclones on my local and started working in that alternate- just mkdir and git init, init is simply initialise 
...
31) Ignoring some files - if some files dont need to be version controlled, use gitignore- Just go to root and create          
        ...
        * matches any number of characters     : *.tmp      means all .tmp files        
        ? matches a single character           : my?ile.txt means matches my1ile.txt, myAile.txt, etc.      
        [abc] matches any character in the set :log[0-9].txt means log1.txt, log2.txt, ... log9.txt 
